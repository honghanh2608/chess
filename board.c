...
static void initialize_board(wchar_t ** p_board) {
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 8; j++) {
      ...
      if (i == 0) {
        if (j == 0 || j == 7) p_board[i][j] = white_rook;//xe 
        if (j == 1 || j == 6) p_board[i][j] = white_knight;//ma	
        if (j == 2 || j == 5) p_board[i][j] = white_bishop;//tinh 
        if (j == 3) p_board[i][j] = white_queen;//hau 
        if (j == 4) p_board[i][j] = white_king;//vua 
      }
...
static char * create_od_board() {//tạo mảng dl thực 
...
static wchar_t translate_piece (char char_piece) {//chuyen doi od_board thanh dl hien thi 
